...
describe('AddIgnoredSeriesAction', () => {
  ...
  beforeEach(async () => {
    ...
    addIgnoredSeriesAction = new AddIgnoredSeriesAction(ignoredSeriesRepository);
  ...
  describe('execute', () => {
    ...
    it('throws ResourceAlreadyExistsError when series is already ignored', async () => {
      ...
      await expect(addIgnoredSeriesAction.execute(user.id, seriesTmdbId)).rejects.toThrow(
